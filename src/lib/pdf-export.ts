...
export function generateWeeklyPDFReport(data: WeeklyReportData) {
  ...
  doc.text('CONSTY', pageWidth / 2, 15, { align: 'center' });
  ...
  doc.setTextColor(107, 114, 128);
  doc.text(
    `Generated by Consty Construction Management System | ${format(new Date(), 'MMM dd, yyyy HH:mm')}`,
    pageWidth / 2,
    footerY,
    { align: 'center' }
  );
  ...
  // Save the PDF
  const filename = `Consty_Weekly_Report_${format(weekStartDate, 'yyyy-MM-dd')}_to_${format(weekEndDate, 'yyyy-MM-dd')}.pdf`;
  doc.save(filename);
}
